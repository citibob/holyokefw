...
public class CloseableTabbedPane extends JTabbedPane {
	...
	static final Icon removeIcon;
	static {
		Class klass = CloseableTabbedPane.class;
		ClassLoader cl = klass.getClassLoader();
		URL url = cl.getResource(klass.getPackage().getName().replace('.', '/') + "/remove.png");
		removeIcon = new javax.swing.ImageIcon(url);
	}
	
	private final static int ICON_TEXT_GAP = 8;
...
//			putValue(AbstractXNapAction.ICON_FILENAME, "remove.png");
	...
	protected class TabTitleComponent extends Box implements TabTitle {
		...
		public TabTitleComponent(String title, Component component,
			...
			closeButton = new TabTitleButton(new DefaultCloseAction(component));
			add(closeButton);
    ...
    protected class TabTitleIcon implements Icon, TabTitle {
		...
		public TabTitleIcon(String title, Component comp, Icon leftIcon)
		{
			...
			this.closeButtonIcon = new EventIcon(new TabTitleButton(new DefaultCloseAction(comp)));
		...
		protected void drawTitleAndRightIcon(Component c, Graphics g, int x, int y, int offset)
		{
			...
			g.setColor((index != -1) 
					? CloseableTabbedPane.this.getForegroundAt(index)
					: CloseableTabbedPane.this.getForegroundAt(index));
